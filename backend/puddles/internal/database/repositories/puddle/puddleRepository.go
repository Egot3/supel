...
type PuddleRepository interface {
	...
	CreateGroup(ctx context.Context, name string, description *string, startingUserUUIDs uuid.UUIDs, ownerUUID uuid.UUID) error
	...
	ListPuddleMembers(ctx context.Context, puddleUUID uuid.UUID, page, size uint32) (uuid.UUIDs, int, error)
