...
func (r *bunLessonRepository) PatchLesson(ctx context.Context, pathced models.LessonPatched) error {
	...
	query := r.db.NewUpdate().Model(&models.Lesson{UUID: pathced.UUID}).WherePK()
	...
	if pathced.Cancelled != nil {
		query = query.Set("cancelled = ?", pathced.Cancelled)
		isUpdated = true
	}
...
func (r *bunLessonRepository) LessonByEntry(ctx context.Context, entryUUId uuid.UUID) (*models.Lesson, error) {
	lesson := models.Lesson{}
	err := r.db.NewSelect().Model(&lesson).Where("timetable_entry_uuid = ?", entryUUId).Scan(ctx)
	if err != nil {
		return nil, err
	}
