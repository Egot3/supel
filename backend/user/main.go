...
import (
	...
	"github.com/Egot3/supel/backend/user/internal/server"
...
func main() {
	...
	s3Config := storage.Config{
		...
		Bucket:    os.Getenv("STORAGE_USER_BUCKET"),
	}
	...
	if err := s3Service.EnsureBuckets(ctx, []string{os.Getenv("STORAGE_USER_BUCKET")}); err != nil {
		log.Fatal(err)
	}

	UserServer := server.NewUserService(*s3Service)
	...
	log.Printf("user Service gRPC server on %s", port)
